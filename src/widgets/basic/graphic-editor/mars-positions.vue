      ...
      <template v-for="(item, i) in lonlats" :key="i">
        <p class="position-title" :span="24">
          ...
          <a-space class="position-title__subfix">
            <mars-icon icon="add-one" width="16" v-if="lonlats.length < maxNum" @click="addItem(item, i)"></mars-icon>
            <mars-icon icon="delete" width="16" v-if="lonlats.length > minNum" @click="deleteItem(i)"></mars-icon>
          </a-space>
        </p>
        <table class="mars-primary-table" border="1" bordercolor="#ffffff" cellspacing="0" cellpadding="0">
          <tr>
...
const emit = defineEmits(["pointChange", "addRemovePoint"])
function pointChange(lonlats: any[]) {
  emit("pointChange", lonlats)
}
function addRemovePoint(lonlats: any[]) {
  emit("addRemovePoint", lonlats)
}
...
watch(
  ...
  () => {
    ...
    minNum.value = props.minNum
    maxNum.value = props.maxNum
  },
  {
    immediate: true
  }
